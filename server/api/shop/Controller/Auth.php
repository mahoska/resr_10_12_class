...
class Auth extends Controller
{
    ...
    public function getAuth($params)
    {
       // return ['status'=>200,'data'=>$param];
         if(count($params) != 2){
             return ['status'=>400, 'data'=>'not all fields are filled in'];
        }
         $model = new AuthModel();
    ...
    public function postAuth($params)
    {
        ...
        /*foreach($params as $param=>$val){
            if(strcmp($val,""))
                return ['status'=>200,'err_param'=>1, 'data'=>'not all fields are filled in'];
    }*/
        $model = new AuthModel();
        $isUser = $model->isLoginExists($params['login']);
        if(!$isUser)
            return ['status'=>200,'err_param'=>1, 'data'=>'not all filds filled or login alredy exists'];
